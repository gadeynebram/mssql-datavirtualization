...
class VirtualizationWizard implements vscode.Disposable{
  ...
  async RunWizard(){
    ...
    const scripts = await this.GenerateExternalTableScripts();
...
ORDER BY s.name, v.name`;
  ...
  // Step 5: Generate external table scripts using schema detection
  private async GenerateExternalTableScripts(): Promise<string> {
    const scripts: string[] = [];
    scripts.push('-- Generated by Data Virtualization Wizard');
    scripts.push(`-- Date: ${new Date().toLocaleString()}`);
    scripts.push(`-- External Data Source: ${this.SelectedExternalDataSource}`);
    scripts.push('');
    ...
    await vscode.window.withProgress({
      ...
      const total = this.SelectedTablesAndViews.length;
      for(let i = 0; i < total; i++){
        const item = this.SelectedTablesAndViews[i];
        ...
        // Probe the schema using the helper
        const detectedSchema = await this.ProbeDetectedSchema(remote, this.SelectedExternalDataSource!, `${item.schema}_${item.name}`);
        
        if(detectedSchema){
          scripts.push(`CREATE EXTERNAL TABLE ${localFullName}`);
          scripts.push(detectedSchema);
          scripts.push(`WITH (LOCATION = N'${remote}', DATA_SOURCE = [${this.SelectedExternalDataSource}]);`);
        } else {
  ...
  // Step 6: Open generated scripts in a new SQL editor
  ...
  // Step 7: Cleanup all DVW discovery tables
  private async CleanupDVWTables(): Promise<void>{
    ...
    await vscode.window.withProgress({
      ...
      const total = this.DiscoveryTables.length;
      for(let i = 0; i < total; i++){
        const fullName = this.DiscoveryTables[i];
        const parts = fullName.split('.');
        if(parts.length === 2){
          progress.report({ increment: (i / total) * 100, message: `Dropping ${parts[1]} (${i+1}/${total})` });
